...
class Attention(evit.Attention):
    def forward(self, x, keep_rate=None, tokens=None, aux_sem=None, return_attn=False):
# forward做了修改，加了return_attn=False
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double-check the keep rate
            ...
            if return_attn:
                return x, index, idx, cls_attn, left_tokens, token_cls, attn
            return x, index, idx, cls_attn, left_tokens, token_cls
...
class Block(evit.Block):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, aux_sem=None, return_attn=False):
        ...
        tmp, index, idx, cls_attn, left_tokens, token_cls, attn = \
            self.attn(self.norm1(x), keep_rate, tokens, aux_sem, return_attn)
        ...
        if get_idx and index is not None:
            return x, n_tokens, idx, token_cls, cls_attn, attn
        return x, n_tokens, None, token_cls, None, None
...
class ZSLViT(evit.EViT):
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False, labels=None, atts=None):
        ...
        for i, blk in enumerate(self.blocks):
            aux_sem = None
            # 判断是否使用 S2V / V2S
            if self.keep_rate[i] < 1 and self.training and atts is not None and p_count < len(self.S2V):
                ...
                x, left_token, idx, token_cls, cls_attn, attn = blk(
                    x, keep_rate[i], tokens[i], get_idx=True, aux_sem=aux_sem, return_attn=True
                )
                ...
                x, left_token, idx, token_cls, cls_attn, attn = blk(
                    x, keep_rate[i], tokens[i], get_idx=True, aux_sem=None, return_attn=True
                )
            ...
            if idx is not None:
                idxs.append(idx)
                attn_maps[i] = cls_attn.detach().cpu() if cls_attn is not None else None
                token_indices[i] = idx.detach().cpu() if idx is not None else None

        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), left_tokens, idxs, aux, token_cls_reg, attn_maps, token_indices
        else:
            return x[:, 0], x[:, 1], idxs

    def forward(self, x, keep_rate=None, tokens=None, get_idx=False, labels=None, atts=None):
        # 接收所有7个返回值
        x, left_tokens, idxs, aux, token_cls_reg, attn_maps, token_indices = self.forward_features(
            x, keep_rate, tokens, get_idx, labels, atts
        )
        ...
        # 返回顺序：x, aux, token_cls_reg, logits, attn_maps, token_indices
        return x, aux, token_cls_reg, logits, attn_maps, token_indices
